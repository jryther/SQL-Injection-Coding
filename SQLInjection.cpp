...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
  ...
  /**
  * If statement used to detect SQL injection in query.
  * Parameter checks query string to see if " or " is present.
  * This substring would only be present on an injection.
  * If true then an error message prints and the function returns false.
  **/
  if (sql.find(flaggedString) != std::string::npos) {
